...
def pad3d(x):
    ...
    xpad = torchnn.functional.pad(x,(0,0,int(dims[1]/2),int(dims[1]/2),int(dims[0]/2),int(dims[0]/2)),mode='constant', value=0)
    return xpad 
...
def pad2d(x):
    ...
    dims = x.shape
    # only pad along last two dimensions, assume x and y
    padder = torchnn.ConstantPad2d((int(dims[1]/2),int(dims[1]/2),int(dims[0]/2),int(dims[0]/2)),0)
    xpad = padder(x)
    return xpad 
    
def crop3d(x):
    #get size of x. crop in half using only 2nd and 3rd quarter
    dims = x.shape
    st0 = int(dims[0]/4)
    st1 = int(dims[1]/4)
    end0 = int(3*dims[0]/4)
    end1 = int(3*dims[1]/4)
    #only crop the last two dimensions
    xcrop = x[st0:end0,st1:end1,:]
    return xcrop
...
def forwardmodel3d(xpad,hfftpad,m):
    ...
    y = torch.real(torch.sum(y, dim=2))
    return y
...
#             print(k)
            ...
            if np.mod(k,kprint)==0 or k==kmax-1:
                ...
                plt.subplot(1,3,1)
                xcrop = crop2d(flatten(xk)).cpu().detach().numpy() #zoom to centerish
                plt.imshow(xcrop)
                plt.title('X Estimate (Zoomed)')
                plt.subplot(1,3,2)
                ycrop = crop2d(yest).cpu().detach().numpy() #zoom to centerish
                plt.imshow(ycrop)
...
# def padPow2(mat):
#     y_pad = torch.tensor(2**np.ceil(np.log2(mat.shape[0]*2))-mat.shape[0])
#     x_pad = torch.tensor(2**np.ceil(np.log2(mat.shape[1]*2))-mat.shape[1])
#     padding = ((torch.ceil(x_pad/2)).type(torch.int),(torch.floor(x_pad/2)).type(torch.int),(torch.ceil(y_pad/2)).type(torch.int),(torch.floor(y_pad/2)).type(torch.int))
...
#     padding = ((torch.ceil(x_pad/2)).type(torch.int),(torch.floor(x_pad/2)).type(torch.int),(torch.ceil(y_pad/2)).type(torch.int),(torch.floor(y_pad/2)).type(torch.int))
#     padder = nn.ZeroPad2d(padding)
#     return padder(mat.permute(2,0,1)).permute(1,2,0)

# def crop3D(target_shape, mat):
#     cropper = torchvision.transforms.CenterCrop(target_shape)
#     return cropper(mat.permute(2,0,1)).permute(1,2,0)
...
def A_op(obj,H,padded_filt):
    ...
    meas = crop2D(obj.shape[0:2],meas)
    return meas.real
...
def Aadj_op(b, padded_psf, filt):
    ...
    return crop3D(psf.shape,x.real)
